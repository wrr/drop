...
class TestPty(TestBase):
    ...
    def test_has_terminal(self):
        ...
        result = self.drop('run tty', stdin=sys.stdin)
        ...
        # ? as the TTY)
        result = self.drop('run ps -o tty=', stdin=sys.stdin)
        self.assertSuccess(result)
    ...
    def test_no_terminal_when_streams_redirected(self):
        ...
        tty_result = self.drop('run tty')
        ps_result = self.drop('run ps -o tty=')
    ...
    def test_only_terminal_fds_are_terminals_in_sandbox(self):
        result = self.drop('run readlink /proc/self/fd/0',
                           stdin=sys.stdin)
        self.assertSuccess(result)
        self.assertEqual('/dev/pts/0', result.stdout.strip())

        # Pipes should not go through terminal
        result = self.drop('run readlink /proc/self/fd/1')
        self.assertSuccess(result)
        self.assertIn('pipe:', result.stdout.strip())

        result = self.drop('run readlink /proc/self/fd/2')
        self.assertSuccess(result)
        self.assertIn('pipe:', result.stdout.strip())
    ...
    def test_ptmx_cannot_be_removed(self):
        ...
        result = self.drop('run rm -rf /dev/ptmx')
